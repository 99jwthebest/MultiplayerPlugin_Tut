...
void UMultiplayerSessionsSubsystem::CreateSession(int32 NumPublicConnections, FString MatchType)
{
	...
	LastSessionSettings->bAllowJoinViaPresence = true; // joins depending on region of the world
	...
	if (!SessionInterface->CreateSession(*LocalPlayer->GetPreferredUniqueNetId(), NAME_GameSession, *LastSessionSettings))
	{
		...
		// Broadcast our own custom delegate so the Menu class knows that session creation has failed.
...
{
	if(!SessionInterface.IsValid())// || !GetGameInstance()->GetFirstLocalPlayerController())
	{
		return;
	}
	...
	LastSessionSearch->QuerySettings.Set(SEARCH_LOBBIES, true, EOnlineComparisonOp::Equals); // for using the region of the world to find sessions
